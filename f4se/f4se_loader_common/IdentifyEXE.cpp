    ...
    if (version < kCurVersion) {
        ...
        if (versionInternal == CURRENT_RELEASE_RUNTIME)
            PrintLoaderError(
              ...
              F4SE_VERSION_INTEGER_MINOR,
              F4SE_VERSION_INTEGER_BETA,
              CURRENT_RELEASE_F4SE_STR);
        else
            PrintLoaderError("You are using Fallout version %d.%d.%d, which is out of date and incompatible with this version of F4SE. Update to the latest beta version.",
                             GET_EXE_VERSION_MAJOR(versionInternal),
                             GET_EXE_VERSION_MINOR(versionInternal),
                             GET_EXE_VERSION_BUILD(versionInternal));
#else
        PrintLoaderError("You are using Fallout version %d.%d.%d, which is out of date and incompatible with this version of F4SE. Update to the latest version.",
                         GET_EXE_VERSION_MAJOR(versionInternal),
                         GET_EXE_VERSION_MINOR(versionInternal),
                         GET_EXE_VERSION_BUILD(versionInternal));
#endif
