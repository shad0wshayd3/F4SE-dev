...
int main(int argc, char** argv) {
    ...
    _MESSAGE("f4se loader %08X %08X%08X %s", PACKED_F4SE_VERSION, now.dwHighDateTime, now.dwLowDateTime, GetOSInfoStr().c_str());
    ...
    if (g_options.m_launchCS) {
        ...
        if (!procName.empty()) {
            ...
            UInt32 procNameCheck = (procName[0] << 8) | (procName[1] << 24) | (procName[2] << 16) | (procName[3] << 0);
    ...
    // check if the exe exists
    {
        ...
        if (!fileCheck.Open(procPath.c_str())) {
            if (usedCustomRuntimeName) {
                // hurr durr
                PrintLoaderError(
                  "Couldn't find %s. You have customized the runtime name via F4SE's .ini file, and that file does not exist. This can usually be fixed by removing the RuntimeName line from the .ini file.)",
                  procName.c_str());
            } else {
    ...
    if (!CreateProcess(procPath.c_str(),
                       NULL,     // no args
