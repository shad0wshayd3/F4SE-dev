...
class RelocPtr {
public:
    RelocPtr(uintptr_t offset): m_offset(offset + RelocationManager::s_baseAddr) {
        //
    }
    ...
    T* GetPtr() const { return reinterpret_cast<T*>(m_offset); }

    const T* GetConst() const { return reinterpret_cast<T*>(m_offset); }

    uintptr_t GetUIntPtr() const { return m_offset; }
...
class RelocAddr {
public:
    RelocAddr(uintptr_t offset): m_offset(reinterpret_cast<BlockConversionType*>(offset + RelocationManager::s_baseAddr)) {
        //
    }

    operator T() { return reinterpret_cast<T>(m_offset); }

    uintptr_t GetUIntPtr() const { return reinterpret_cast<uintptr_t>(m_offset); }
