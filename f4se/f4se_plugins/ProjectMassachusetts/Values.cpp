...
void Values::RegisterValues() {
    ...
    for (int i = 0; i < NPCList.count; i++) {
        ...
        for (int j = thisNPC->perkRankArray.numPerkRanks - 1; j >= 0; j--)
            Forms::RegisteredActors.back().insert(Forms::RegisteredActors.back().begin(), BGSPerkRankArray::Data{thisNPC->perkRankArray.perkRanks[j].perk, thisNPC->perkRankArray.perkRanks[j].rank});
