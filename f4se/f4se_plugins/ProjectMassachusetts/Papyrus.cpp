...
bool Papyrus::Init(VirtualMachine* VM) {
    ...
    VM->RegisterFunction(new NativeFunction2<StaticFunctionTag, VMArray<UInt32>, BGSListForm*, TESForm*>("FindNestedForm", PAPYRUS_SCRIPT_NAME, FindNestedForm, VM));

    VM->RegisterFunction(new NativeFunction0<StaticFunctionTag, void>("RemoveAllTraits", PAPYRUS_SCRIPT_NAME, RemoveAllTraits, VM));

    // Override existing Game.AdvanceSkill and Game.IncrementSkill functions
    VM->RegisterFunction(new NativeFunction2<StaticFunctionTag, void, BSFixedString, float>("AdvanceSkill", "Game", AdvanceSkill, VM));

    VM->RegisterFunction(new NativeFunction2<StaticFunctionTag, void, ActorValueInfo*, UInt32>("IncrementSkill", "Game", IncrementSkill, VM));
