...
void papyrusMaterialSwap::RegisterFuncs(VirtualMachine* vm) {
    ...
    vm->RegisterFunction(new NativeFunction0<BGSMaterialSwap, VMArray<RemapData>>("GetRemapData", "MatSwap", papyrusMaterialSwap::GetRemapData, vm));

    vm->RegisterFunction(new NativeFunction1<BGSMaterialSwap, void, VMArray<RemapData>>("SetRemapData", "MatSwap", papyrusMaterialSwap::SetRemapData, vm));
}
