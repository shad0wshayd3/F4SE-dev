    ...
    if (data) {
        ...
        if (cmd == RID_INPUT) {
            ...
            case RIM_TYPEKEYBOARD: {
                ...
                _MESSAGE("kbd: %04X %04X %04X %04X %08X %08X", kbd->MakeCode, kbd->Flags, kbd->Reserved, kbd->VKey, kbd->Message, kbd->ExtraInformation);
            } break;

            case RIM_TYPEMOUSE: {
                RAWMOUSE* mse = &input->data.mouse;

                _MESSAGE("mse: %04X %08X %08X %d %d %08X", mse->usFlags, mse->ulButtons, mse->ulRawButtons, mse->lLastX, mse->lLastY, mse->ulExtraInformation);
            } break;
            }
        }
    }
...
void HandleButtonEvent(ButtonEvent* inputEvent) {
    ...
    if (isDown) {
        g_inputKeyEventRegs.ForEach(keyCode, [&keyCode](const EventRegistration<NullParameters>& reg) { SendPapyrusEvent1<UInt32>(reg.handle, reg.scriptName, "OnKeyDown", keyCode); });
        g_inputControlEventRegs.ForEach(control, [&control](const EventRegistration<NullParameters>& reg) { SendPapyrusEvent1<BSFixedString>(reg.handle, reg.scriptName, "OnControlDown", control); });
    } else if (isUp) {
        g_inputKeyEventRegs.ForEach(keyCode,
                                    [&keyCode, &timer](const EventRegistration<NullParameters>& reg) { SendPapyrusEvent2<UInt32, float>(reg.handle, reg.scriptName, "OnKeyUp", keyCode, timer); });
        g_inputControlEventRegs.ForEach(control, [&control, &timer](const EventRegistration<NullParameters>& reg) {
