...
// :: f4se-dev ::
// RegistrationMapHolder and RegistrationSetHolder had every reference to SafeDataHolder members prefixed with 'this->', fixes C2065
// RegistrationMapHolder and RegistrationSetHolder had every instance of RegSet::iterator changed to 'auto', fixes C2760
...
enum InternalEventVersion { kVersion1, kVersion2, kCurrentVersion = kVersion2 };
...
public:
    ...
    void AddFilter(TESForm* form) { forms.insert(form ? form->formID : 0); }

    void RemoveFilter(TESForm* form) { forms.erase(form ? form->formID : 0); }

    bool HasFilter(TESForm* form) const { return forms.find(form ? form->formID : 0) != forms.end(); }

    bool NoFilter() const { return forms.size() == 0; }

    void Dump(void) { _MESSAGE("> formId:\t%08X", forms.size()); }
};
...
public:
    ...
    bool Save(const F4SESerializationInterface* intfc, UInt32 version) const { return Serialization::WriteData<BSFixedString>(intfc, &callbackName); }

    bool Load(const F4SESerializationInterface* intfc, UInt32 version) { return Serialization::ReadData<BSFixedString>(intfc, &callbackName); }

    void Dump(void) { _MESSAGE("> callbackName:\t%s", callbackName); }
};
