...
class BSReadWriteLock {
    enum { kFastSpinThreshold = 10000, kLockWrite = 0x80000000, kLockCountMask = 0xFFFFFFF };
...
class BSReadLocker {
public:
    BSReadLocker(BSReadWriteLock* lock) {
        m_lock = lock;
        m_lock->LockForRead();
    }
...
class BSWriteLocker {
public:
    BSWriteLocker(BSReadWriteLock* lock) {
        m_lock = lock;
        m_lock->LockForWrite();
    }
...
public:
    BSReadAndWriteLocker(BSReadWriteLock* lock) {
        m_lock = lock;
        m_lock->LockForReadAndWrite();
    }
...
class SimpleLocker {
public:
    SimpleLocker(SimpleLock* dataHolder) {
        m_lock = dataHolder;
        m_lock->Lock();
    }
...
public:
    // 18+
    struct Entry {
        enum { kState_RefcountMask = 0x3FFF, kState_External = 0x4000, kState_Wide = 0x8000 };
...
public:
    ...
    class AcceptAll {
    public:
        bool Accept(T* item) { return true; }
    };

    void RemoveAll() { FreeNodes(AcceptAll()); }
    ...
    public:
        ...
        bool Accept(T* a_item) { return *item == *a_item; }
    };

    bool Contains(T* item) const { return Find(AcceptEqual(item)) != NULL; }
...
class tHashSet {
    ...
    enum InsertResult { kInsert_Duplicate = -1, kInsert_OutOfSpace = 0, kInsert_Success = 1 };
...
public:
    ...
    // T& -> T, causes C2664 on c++17 or higher.
    template<typename T>
