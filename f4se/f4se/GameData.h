...
struct ModInfo    // referred to by game as TESFile
{
    ...
    enum RecordFlag { kRecordFlags_None = 0, kRecordFlags_ESM = 1 << 0, kRecordFlags_Active = 1 << 3, kRecordFlags_Localized = 1 << 7, kRecordFlags_ESL = 1 << 9 };
    ...
    UInt32 GetPartialIndex() const { return !IsLight() ? modIndex : (0xFE000 | lightIndex); }

    // Converts the lower bits of a FormID to a full FormID depending on plugin type
    UInt32 GetFormID(UInt32 formLower) const { return !IsLight() ? UInt32(modIndex) << 24 | (formLower & 0xFFFFFF) : 0xFE000000 | (UInt32(lightIndex) << 12) | (formLower & 0xFFF); }
...
class LocationData {
public:
    LocationData(Actor* actor) { CALL_MEMBER_FN(this, ctor)(actor); }
    ~LocationData(){};
...
public:
    ...
    enum Favorites { kNumFavorites = 12 };
