...
namespace papyrusObjectReference {
    ...
    float GetInventoryWeight(TESObjectREFR* refr) { return refr ? CALL_MEMBER_FN(refr, GetInventoryWeight)() : 0.0f; }
    ...
    struct CompareMaterial {
        // Const Method, fixes C3848.
        bool operator()(const std::pair<BGSMaterialSwap::MaterialSwap*, float> lhs, const std::pair<BGSMaterialSwap::MaterialSwap*, float> rhs) const { return lhs.first->source < rhs.first->source; }
    };
...
void papyrusObjectReference::RegisterFuncs(VirtualMachine* vm) {
    ...
    vm->RegisterFunction(new NativeFunction0<VMRefOrInventoryObj, VMArray<BGSMod::Attachment::Mod*>>("GetAllMods", "ObjectReference", papyrusObjectReference::GetAllMods, vm));

    vm->RegisterFunction(new NativeFunction0<TESObjectREFR, VMArray<TESObjectREFR*>>("GetConnectedObjects", "ObjectReference", papyrusObjectReference::GetConnectedObjects, vm));

    vm->RegisterFunction(new LatentNativeFunction2<TESObjectREFR, TESObjectREFR*, TESObjectREFR*, TESForm*>("AttachWire", "ObjectReference", papyrusObjectReference::AttachWire, vm));

    vm->RegisterFunction(new NativeFunction0<VMRefOrInventoryObj, BSFixedString>("GetDisplayName", "ObjectReference", papyrusObjectReference::GetDisplayName, vm));

    vm->RegisterFunction(new LatentNativeFunction0<TESObjectREFR, VMArray<TESForm*>>("GetInventoryItems", "ObjectReference", papyrusObjectReference::GetInventoryItems, vm));

    vm->RegisterFunction(new NativeFunction0<TESObjectREFR, float>("GetInventoryWeight", "ObjectReference", papyrusObjectReference::GetInventoryWeight, vm));

    vm->RegisterFunction(new NativeFunction1<VMRefOrInventoryObj, void, BGSMaterialSwap*>("SetMaterialSwap", "ObjectReference", papyrusObjectReference::SetMaterialSwap, vm));

    vm->RegisterFunction(new NativeFunction0<VMRefOrInventoryObj, BGSMaterialSwap*>("GetMaterialSwap", "ObjectReference", papyrusObjectReference::GetMaterialSwap, vm));

    vm->RegisterFunction(new LatentNativeFunction0<TESObjectREFR, VMArray<ConnectPoint>>("GetConnectPoints", "ObjectReference", papyrusObjectReference::GetConnectPoints, vm));

    vm->RegisterFunction(new LatentNativeFunction0<TESObjectREFR, bool>("TransmitConnectedPower", "ObjectReference", papyrusObjectReference::TransmitConnectedPower, vm));
...
#ifdef _DEBUG
    vm->RegisterFunction(new LatentNativeFunction0<TESObjectREFR, void>("Scrap", "ObjectReference", papyrusObjectReference::Scrap, vm));
