...
namespace papyrusActorBase {
    ...
    bool HasHeadPartOverlays(TESNPC* thisNPC) { return CALL_MEMBER_FN(thisNPC, HasOverlays)(); }
...
void papyrusActorBase::RegisterFuncs(VirtualMachine* vm) {
    vm->RegisterFunction(new NativeFunction1<TESNPC, TESNPC*, bool>("GetTemplate", "ActorBase", papyrusActorBase::GetTemplate, vm));

    vm->RegisterFunction(new NativeFunction0<TESNPC, bool>("HasHeadPartOverlays", "ActorBase", papyrusActorBase::HasHeadPartOverlays, vm));

    vm->RegisterFunction(new NativeFunction1<TESNPC, VMArray<BGSHeadPart*>, bool>("GetHeadParts", "ActorBase", papyrusActorBase::GetHeadParts, vm));

    vm->RegisterFunction(new NativeFunction1<TESNPC, BGSOutfit*, bool>("GetOutfit", "ActorBase", papyrusActorBase::GetOutfit, vm));

    vm->RegisterFunction(new NativeFunction1<TESNPC, void, BodyWeight>("SetBodyWeight", "ActorBase", papyrusActorBase::SetBodyWeight, vm));

    vm->RegisterFunction(new NativeFunction0<TESNPC, BodyWeight>("GetBodyWeight", "ActorBase", papyrusActorBase::GetBodyWeight, vm));
