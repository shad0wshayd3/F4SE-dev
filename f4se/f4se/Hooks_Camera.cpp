...
void SetCameraState_Hook(TESCamera* camera, TESCameraState* newCameraState) {
    if (camera == (*g_playerCamera)) {
        ...
        g_cameraEventRegs.ForEach(
          [&oldState, &newState](const EventRegistration<NullParameters>& reg) { SendPapyrusEvent2<SInt32, SInt32>(reg.handle, reg.scriptName, "OnPlayerCameraState", oldState, newState); });
    }
