...
static const F4SEInterface g_F4SEInterface = {PACKED_F4SE_VERSION,
...
static const F4SEScaleformInterface g_F4SEScaleformInterface = {F4SEScaleformInterface::kInterfaceVersion, RegisterScaleformPlugin};

#include "Hooks_Papyrus.h"

static const F4SEPapyrusInterface g_F4SEPapyrusInterface = {F4SEPapyrusInterface::kInterfaceVersion, RegisterPapyrusPlugin, GetExternalEventRegistrations};

#include "f4se/Serialization.h"

static const F4SESerializationInterface g_F4SESerializationInterface = {F4SESerializationInterface::kInterfaceVersion,
...
static const F4SETaskInterface g_F4SETaskInterface = {F4SETaskInterface::kInterfaceVersion,
...
static const F4SEObjectInterface g_F4SEObjectInterface = {F4SEObjectInterface::kInterfaceVersion, F4SEDelayFunctorManagerInstance, F4SEObjectRegistryInstance, F4SEObjectStorageInstance};
    ...
    for (IDirectoryIterator iter(m_pluginDirectory.c_str(), "*.dll"); !iter.Done(); iter.Next()) {
        ...
        if (plugin.handle) {
            ...
            if (plugin.query && plugin.load) {
                ...
                _MESSAGE("plugin %s (%08X %s %08X) %s", pluginPath.c_str(), plugin.info.infoVersion, plugin.info.name ? plugin.info.name : "<NULL>", plugin.info.version, loadStatus);
            } else {
...
static const MinVersionEntry kMinVersionList[] = {{NULL, 0, NULL}};
