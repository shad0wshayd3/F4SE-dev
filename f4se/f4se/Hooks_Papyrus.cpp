...
void GetExternalEventRegistrations(const char* eventName, void* data, F4SEPapyrusInterface::RegistrantFunctor functor) {
    g_externalEventRegs.ForEach(BSFixedString(eventName),
                                [&functor, &data](const EventRegistration<ExternalEventParameters>& reg) { functor(reg.handle, reg.scriptName.c_str(), reg.params.callbackName.c_str(), data); });
}
