...
void papyrusActor::RegisterFuncs(VirtualMachine* vm) {
    ...
    vm->RegisterFunction(new NativeFunction2<Actor, void, bool, UInt32>("QueueUpdate", "Actor", papyrusActor::QueueUpdate, vm));

    vm->RegisterFunction(new NativeFunction0<Actor, TESObjectREFR*>("GetFurnitureReference", "Actor", papyrusActor::GetFurnitureReference, vm));
}
