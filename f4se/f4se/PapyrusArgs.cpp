...
template<>
void UnpackValue<float>(float* dst, VMValue* src) {
...
template<>
void UnpackValue<UInt32>(UInt32* dst, VMValue* src) {
    switch (src->type.value) {
    case VMValue::kType_Int:
        *dst = src->data.u;
        break;
...
template<>
void UnpackValue<SInt32>(SInt32* dst, VMValue* src) {
    switch (src->type.value) {
    case VMValue::kType_Int:
        *dst = src->data.u;
        break;
...
template<>
void UnpackValue<bool>(bool* dst, VMValue* src) {
    switch (src->type.value) {
    case VMValue::kType_Int:
        *dst = src->data.u != 0;
        break;
...
template<>
void UnpackValue<BSFixedString>(BSFixedString* dst, VMValue* src) {
...
template<>
void UnpackValue<VMVariable>(VMVariable* dst, VMValue* src) {
    dst->UnpackVariable(src);
}

template<>
void UnpackValue<VMObject>(VMObject* dst, VMValue* src) {
    dst->UnpackObject(src);
}

template<>
void UnpackValue<VMRefOrInventoryObj>(VMRefOrInventoryObj* dst, VMValue* src) {
    dst->UnpackObject(src);
}

template<>
void UnpackValue<VMArray<UInt32>>(VMArray<UInt32>* dst, VMValue* src) {
    UnpackArray(dst, src, VMValue::kType_IntArray);
}

template<>
void UnpackValue<VMArray<SInt32>>(VMArray<SInt32>* dst, VMValue* src) {
    UnpackArray(dst, src, VMValue::kType_IntArray);
}

template<>
void UnpackValue<VMArray<float>>(VMArray<float>* dst, VMValue* src) {
    UnpackArray(dst, src, VMValue::kType_FloatArray);
}

template<>
void UnpackValue<VMArray<bool>>(VMArray<bool>* dst, VMValue* src) {
    UnpackArray(dst, src, VMValue::kType_BoolArray);
}

template<>
void UnpackValue<VMArray<BSFixedString>>(VMArray<BSFixedString>* dst, VMValue* src) {
    UnpackArray(dst, src, VMValue::kType_StringArray);
}

template<>
void UnpackValue<VMArray<VMVariable>>(VMArray<VMVariable>* dst, VMValue* src) {
    UnpackArray(dst, src, VMValue::kType_VariableArray);
}

template<>
void UnpackValue<VMArray<VMObject>>(VMArray<VMObject>* dst, VMValue* src) {
    UnpackArray(dst, src, GetTypeID<VMArray<VMObject>>((*g_gameVM)->m_virtualMachine));
}

template<>
void UnpackValue<VMArray<VMRefOrInventoryObj>>(VMArray<VMRefOrInventoryObj>* dst, VMValue* src) {
    UnpackArray(dst, src, GetTypeID<VMArray<VMRefOrInventoryObj>>((*g_gameVM)->m_virtualMachine));
}

template<>
void UnpackValue(VMObject** dst, VMValue* src) {
...
template<>
void DestroyValue(VMObject** dst) {
    if (*dst)
        delete (*dst);
}

template<>
void UnpackValue(VMRefOrInventoryObj** dst, VMValue* src) {
    if (src->IsIdentifier() && src->data.id) {
...
template<>
void DestroyValue(VMRefOrInventoryObj** dst) {
    if (*dst)
        delete (*dst);
}
...
void* UnpackHandle(VMValue* src, UInt32 typeID) {
    ...
    if (!(*g_objectHandlePolicy)->IsType(typeID, handle))
        return NULL;
    if (!(*g_objectHandlePolicy)->Unk_03(handle))
        return NULL;
...
template<>
UInt64 GetTypeID<int>(VirtualMachine* vm) {
    return VMValue::kType_Int;
}
template<>
...
template<>
UInt64 GetTypeID<VMArray<int>>(VirtualMachine* vm) {
    return VMValue::kType_IntArray;
}
template<>
...
template<>
UInt64 GetTypeID<VMRefOrInventoryObj>(VirtualMachine* vm) {
    return GetTypeIDFromFormTypeID(VMRefOrInventoryObj::kTypeID, vm);
}

template<>
UInt64 GetTypeID<VMArray<VMRefOrInventoryObj>>(VirtualMachine* vm) {
