...
namespace papyrusUI {
    ...
    bool IsMenuOpen(StaticFunctionTag*, BSFixedString menuName) { return (*g_ui)->IsMenuOpen(menuName); }
    ...
    bool IsMenuRegistered(StaticFunctionTag*, BSFixedString menuName) { return (*g_ui)->IsMenuRegistered(menuName); }
    ...
    public:
        F4SEScaleform_OnLoadCompleted(UInt64 handle, BSFixedString objectType, BSFixedString callback, BSFixedString menuName, BSFixedString varPath, BSFixedString assetPath):
            _handle(handle), _objectType(objectType), _callback(callback), _menuName(menuName), _varPath(varPath), _assetPath(assetPath) {}
    ...
    public:
        F4SEScaleform_OnLoadFailed(UInt64 handle, BSFixedString objectType, BSFixedString callback, BSFixedString menuName, BSFixedString varPath, BSFixedString assetPath):
            _handle(handle), _objectType(objectType), _callback(callback), _menuName(menuName), _varPath(varPath), _assetPath(assetPath) {}
...
void papyrusUI::RegisterFuncs(VirtualMachine* vm) {
    ...
    vm->RegisterFunction(new LatentNativeFunction2<StaticFunctionTag, VMVariable, BSFixedString, BSFixedString>("Get", "UI", papyrusUI::Get, vm));

    vm->RegisterFunction(new LatentNativeFunction3<StaticFunctionTag, VMVariable, BSFixedString, BSFixedString, VMArray<VMVariable>>("Invoke", "UI", papyrusUI::Invoke, vm));

    vm->RegisterFunction(new LatentNativeFunction5<StaticFunctionTag, bool, BSFixedString, BSFixedString, BSFixedString, VMObject, BSFixedString>("Load", "UI", papyrusUI::Load, vm));

    vm->RegisterFunction(new NativeFunction1<StaticFunctionTag, bool, BSFixedString>("IsMenuOpen", "UI", papyrusUI::IsMenuOpen, vm));

    vm->RegisterFunction(new NativeFunction1<StaticFunctionTag, bool, BSFixedString>("OpenMenu", "UI", papyrusUI::OpenMenu, vm));

    vm->RegisterFunction(new NativeFunction1<StaticFunctionTag, bool, BSFixedString>("CloseMenu", "UI", papyrusUI::CloseMenu, vm));

    vm->RegisterFunction(new NativeFunction1<StaticFunctionTag, bool, BSFixedString>("IsMenuRegistered", "UI", papyrusUI::IsMenuRegistered, vm));

    vm->RegisterFunction(new NativeFunction4<StaticFunctionTag, bool, BSFixedString, BSFixedString, BSFixedString, MenuData>("RegisterCustomMenu", "UI", papyrusUI::RegisterCustomMenu, vm));
