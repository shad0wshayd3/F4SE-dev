...
class IMemPool {
public:
    IMemPool(): m_free(NULL), m_alloc(NULL) { Reset(); }
    ...
    bool Full(void) { return m_free == NULL; }

    bool Empty(void) { return m_alloc == NULL; }
...
class IBasicMemPool {
public:
    IBasicMemPool(): m_free(NULL) { Reset(); }
    ...
    T* GetByID(UInt32 id) { return m_items[id].GetObj(); }
...
public:
    IThreadSafeBasicMemPool(): m_free(NULL) { Reset(); }
