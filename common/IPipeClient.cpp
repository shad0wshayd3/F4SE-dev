...
IPipeClient::IPipeClient(): m_pipe(INVALID_HANDLE_VALUE) {
    //
}
...
bool IPipeClient::Open(const char* name) {
    ...
    m_pipe = CreateFile(name, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);
...
bool IPipeClient::ReadMessage(UInt8* buf, UInt32 length) {
    ...
    return (bytesRead >= sizeof(IPipeServer::MessageHeader)) &&    // has a valid header
