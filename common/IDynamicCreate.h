...
#define DYNAMIC_INSTANTIATE_HANDLER(name)                                                                                                                                                              \
    IDynamic* name## ::__DYN_##name##_DynamicType::Instantiate(IDataStream* stream) {                                                                                                                  \
        name* object = new name;
#define END_DYNAMIC_INSTANTIATE_HANDLER                                                                                                                                                                \
    return object;                                                                                                                                                                                     \
    }

//! Specifies that a dynamic class should not be instantiated automatically
#define NO_DYNAMIC_INSTANTIATE_HANDLER(name)                                                                                                                                                           \
    DYNAMIC_INSTANTIATE_HANDLER(name) { HALT("attempted to instantiate " #name); }                                                                                                                     \
    END_DYNAMIC_INSTANTIATE_HANDLER
...
public:
    ...
    static IDynamic* Create(UInt32 id) {
        IDynamicType* info = LookupClassInfo(id);
        return info ? info->Create() : NULL;
    }
    static IDynamic* Create(char* name) {
        IDynamicType* info = LookupClassInfo(name);
        return info ? info->Create() : NULL;
    }

    static IDynamic* Instantiate(UInt32 id, IDataStream* stream) {
        IDynamicType* info = LookupClassInfo(id);
        return info ? info->Instantiate(stream) : NULL;
    }
    static IDynamic* Instantiate(char* name, IDataStream* stream) {
        IDynamicType* info = LookupClassInfo(name);
        return info ? info->Instantiate(stream) : NULL;
    }

    static char* GetName(UInt32 id) {
        IDynamicType* info = LookupClassInfo(id);
        return info ? info->GetName() : NULL;
    }
