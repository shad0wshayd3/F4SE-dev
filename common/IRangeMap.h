...
public:
    struct Entry {
        bool Contains(t_key addr, t_key base) { return (addr >= base) && (addr <= (base + length - 1)); }
    ...
    typename EntryMapType::iterator Begin(void) { return m_entries.begin(); }

    typename EntryMapType::iterator End(void) { return m_entries.end(); }
