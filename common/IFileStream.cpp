...
IFileStream::IFileStream(): theFile(INVALID_HANDLE_VALUE) {}

IFileStream::IFileStream(const char* name): theFile(INVALID_HANDLE_VALUE) {
    Open(name);
}
...
bool IFileStream::BrowseCreate(const char* defaultName, const char* defaultPath, const char* title) {
    ...
    info.Flags = OFN_EXPLORER | OFN_ENABLESIZING | OFN_ENABLEHOOK | OFN_NOCHANGEDIR | OFN_OVERWRITEPROMPT | OFN_PATHMUSTEXIST;
    info.lpstrDefExt = NULL;
    info.lCustData = NULL;
    info.lpfnHook = BrowseEventProc;
    info.lpTemplateName = NULL;
